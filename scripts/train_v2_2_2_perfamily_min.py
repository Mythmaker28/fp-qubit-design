...
# -*- coding: utf-8 -*-
"""
Training script for v2.2.2 - Per-Family Minimal
Uses separate models for each major family (Calcium, Voltage, Other)
"""
...
def prepare_features_and_target(df):
    ...
    # Groups for CV (original families)
    ...
    for family in groups:
        if family == "Calcium":
            major_groups.append("Calcium")
        elif family == "Voltage":
            major_groups.append("Voltage")
        else:
            major_groups.append("Other")
    
    major_groups = np.array(major_groups)
    ...
    print(f"Major groups: {len(set(major_groups))} (Calcium: {sum(major_groups == 'Calcium')}, Voltage: {sum(major_groups == 'Voltage')}, Other: {sum(major_groups == 'Other')})")
    print(f"Sample weights range: {sample_weights.min():.3f} - {sample_weights.max():.3f}")
    
    return X, y_original, y_log, groups, major_groups, sample_weights
...
def train_per_family_models(X, y_log, groups, major_groups, sample_weights):
    """Train separate models for each major family"""
    ...
    for fold in range(5):
        ...
        major_groups_train = major_groups[train_mask]
        major_groups_test = major_groups[test_mask]
        
        # Fit preprocessor
        X_train_processed = preprocessor.fit_transform(X_train)
        X_test_processed = preprocessor.transform(X_test)
        
        # Train models for each major group
        models = {}
        for group in ['Calcium', 'Voltage', 'Other']:
            group_mask_train = major_groups_train == group
            if np.sum(group_mask_train) > 0:  # Only train if we have samples
                ...
                X_group_train = X_train_processed[group_mask_train]
                y_group_train = y_train[group_mask_train]
                weights_group_train = weights_train[group_mask_train]
                
                rf.fit(X_group_train, y_group_train, sample_weight=weights_group_train)
                models[group] = rf
                print(f"  Trained {group} model: {np.sum(group_mask_train)} samples")
            else:
                models[group] = None
                print(f"  No {group} samples in training fold")
        
        # Predict using appropriate model for each test sample
        y_pred_log = np.zeros_like(y_test)
        for i, group in enumerate(major_groups_test):
            if models[group] is not None:
                y_pred_log[i] = models[group].predict(X_test_processed[i:i+1])[0]
            else:
                # Fallback: use mean of training data for this group
                group_mask_train = major_groups_train == group
                if np.sum(group_mask_train) > 0:
                    y_pred_log[i] = np.mean(y_train[group_mask_train])
                else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='FP-DESIGN v2.2.2 Per-Family Minimal')
    parser.add_argument('--data', required=True, help='Path to balanced training data CSV')
    parser.add_argument('--out', required=True, help='Output directory')
    
    args = parser.parse_args()
    
    print("=== FP-DESIGN v2.2.2 PER-FAMILY MINIMAL ===")
    ...
    # Prepare features
    X, y_original, y_log, groups, major_groups, sample_weights = prepare_features_and_target(df)
    
    # Train per-family models with CV
    cv_results = train_per_family_models(X, y_log, groups, major_groups, sample_weights)
    ...
    # Final status
    print(f"\n=== FINAL STATUS ===")
    print(f"Data: N_rows={len(df)} ; Families={len(set(groups))} ; Other={sum(major_groups == 'Other')}")
