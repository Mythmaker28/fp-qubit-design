...
# -*- coding: utf-8 -*-
"""
Training script for v2.2.2 - Router 2-Models with CQR
Uses separate ExtraTrees for Calcium vs Other families with CQR per fold
"""
...
def train_router2_models_with_cqr_cv(X, y_log, groups, sample_weights):
    """Train separate ExtraTrees for Calcium vs Other with CQR per fold"""
    print("\n=== TRAINING ROUTER 2-MODELS WITH CQR CV ===")
    ...
    for fold in range(5):
        ...
        # Train Calcium model
        calcium_mask_subtrain = groups_subtrain == 'Calcium'
        other_mask_subtrain = groups_subtrain != 'Calcium'
        
        print(f"  Calcium samples: {np.sum(calcium_mask_subtrain)}, Other samples: {np.sum(other_mask_subtrain)}")
        
        # Train Calcium model
        if np.sum(calcium_mask_subtrain) > 0:
            ...
            et_calcium.fit(
                X_subtrain_processed[calcium_mask_subtrain], 
                y_subtrain[calcium_mask_subtrain], 
                sample_weight=w_subtrain[calcium_mask_subtrain]
            )
        else:
            et_calcium = None
            print("  No Calcium samples in subtrain")
        
        # Train Other model
        if np.sum(other_mask_subtrain) > 0:
            ...
            et_other.fit(
                X_subtrain_processed[other_mask_subtrain], 
                y_subtrain[other_mask_subtrain], 
                sample_weight=w_subtrain[other_mask_subtrain]
            )
        else:
            et_other = None
            print("  No Other samples in subtrain")
        
        # Predict on calibration set using appropriate model
        y_cal_pred_log = np.zeros_like(y_cal)
        for i, family in enumerate(groups_cal):
            if family == 'Calcium' and et_calcium is not None:
                y_cal_pred_log[i] = et_calcium.predict(X_cal_processed[i:i+1])[0]
            elif family != 'Calcium' and et_other is not None:
                y_cal_pred_log[i] = et_other.predict(X_cal_processed[i:i+1])[0]
            else:
                # Fallback: use mean of training data for this group
                if family == 'Calcium' and np.sum(calcium_mask_subtrain) > 0:
                    y_cal_pred_log[i] = np.mean(y_subtrain[calcium_mask_subtrain])
                elif family != 'Calcium' and np.sum(other_mask_subtrain) > 0:
                    y_cal_pred_log[i] = np.mean(y_subtrain[other_mask_subtrain])
                else:
                    y_cal_pred_log[i] = np.mean(y_subtrain)
        
        # Convert calibration predictions to original scale
        ...
        y_test_pred_log = np.zeros_like(y_test)
        for i, family in enumerate(groups_test):
            if family == 'Calcium' and et_calcium is not None:
                y_test_pred_log[i] = et_calcium.predict(X_test_processed[i:i+1])[0]
            elif family != 'Calcium' and et_other is not None:
                y_test_pred_log[i] = et_other.predict(X_test_processed[i:i+1])[0]
            else:
                # Fallback: use mean of training data for this group
                if family == 'Calcium' and np.sum(calcium_mask_subtrain) > 0:
                    y_test_pred_log[i] = np.mean(y_subtrain[calcium_mask_subtrain])
                elif family != 'Calcium' and np.sum(other_mask_subtrain) > 0:
                    y_test_pred_log[i] = np.mean(y_subtrain[other_mask_subtrain])
                else:
                    y_test_pred_log[i] = np.mean(y_subtrain)
...
def calculate_router2_metrics(cv_results, all_predictions, all_true, y_original):
    """Calculate overall metrics and CQR-specific metrics"""
    print("\n=== CALCULATING ROUTER2 METRICS ===")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='FP-DESIGN v2.2.2 Router 2-Models')
    parser.add_argument('--data', required=True, help='Path to balanced training data CSV')
    parser.add_argument('--out', required=True, help='Output directory')
    
    args = parser.parse_args()
    
    print("=== FP-DESIGN v2.2.2 ROUTER 2-MODELS (CALCIUM vs OTHER) ===")
    ...
    # Train router 2-models with CQR CV
    cv_results, all_predictions, all_true, all_families, all_folds = train_router2_models_with_cqr_cv(X, y_log, groups, sample_weights)
    
    # Calculate router2 metrics
    metrics = calculate_router2_metrics(cv_results, all_predictions, all_true, y_original)
