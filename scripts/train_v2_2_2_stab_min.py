...
# -*- coding: utf-8 -*-
"""
Training script for v2.2.2 - Stabilized with Winsorization and Family Standardization
Uses winsorization on train-only and family-wise standardization for stability
"""
...
def train_model_with_stabilization_cv(X, y_log, groups, sample_weights):
    """Train RandomForest with winsorization and family standardization"""
    print("\n=== TRAINING MODEL WITH STABILIZATION CV ===")
    ...
    # Cross-validation predictions with stabilization
    cv_results = []
    all_predictions = []
    all_true = []
    all_families = []
    ...
    for fold in range(5):
        ...
        X_subtrain, X_cal, y_subtrain, y_cal, w_subtrain, w_cal, groups_subtrain, groups_cal = train_test_split(
            X_train, y_train, weights_train, groups_train, test_size=0.2, random_state=1337
        )
        
        # Winsorization on subtrain only (cap at 99th percentile)
        cap = np.quantile(y_subtrain, 0.99)
        y_subtrain_cap = np.clip(y_subtrain, None, cap)
        print(f"  Winsorization cap: {cap:.3f}")
        
        # Family-wise standardization on subtrain
        family_stats = {}
        for family in np.unique(groups_subtrain):
            family_mask = groups_subtrain == family
            if np.sum(family_mask) > 1:  # Need at least 2 samples for std
                family_y = y_subtrain_cap[family_mask]
                family_stats[family] = {
                    'mean': np.mean(family_y),
                    'std': np.std(family_y)
                }
            else:
                # Fallback to global stats if family has only 1 sample
                family_stats[family] = {
                    'mean': np.mean(y_subtrain_cap),
                    'std': np.std(y_subtrain_cap)
                }
        
        # Apply standardization to subtrain
        y_subtrain_std = np.zeros_like(y_subtrain_cap)
        for i, family in enumerate(groups_subtrain):
            stats = family_stats[family]
            y_subtrain_std[i] = (y_subtrain_cap[i] - stats['mean']) / stats['std']
        
        # Apply same standardization to calibration and test
        y_cal_std = np.zeros_like(y_cal)
        for i, family in enumerate(groups_cal):
            if family in family_stats:
                stats = family_stats[family]
                y_cal_std[i] = (y_cal[i] - stats['mean']) / stats['std']
            else:
                # Use global stats for unseen families
                y_cal_std[i] = (y_cal[i] - np.mean(y_subtrain_cap)) / np.std(y_subtrain_cap)
        
        y_test_std = np.zeros_like(y_test)
        for i, family in enumerate(groups_test):
            if family in family_stats:
                stats = family_stats[family]
                y_test_std[i] = (y_test[i] - stats['mean']) / stats['std']
            else:
                # Use global stats for unseen families
                y_test_std[i] = (y_test[i] - np.mean(y_subtrain_cap)) / np.std(y_subtrain_cap)
        
        # Fit preprocessor and model on standardized subtrain
        X_subtrain_processed = preprocessor.fit_transform(X_subtrain)
        X_cal_processed = preprocessor.transform(X_cal)
        X_test_processed = preprocessor.transform(X_test)
        
        rf.fit(X_subtrain_processed, y_subtrain_std, sample_weight=w_subtrain)
        
        # Predict on calibration set
        y_cal_pred_std = rf.predict(X_cal_processed)
        
        # De-standardize calibration predictions
        y_cal_pred_destd = np.zeros_like(y_cal_pred_std)
        for i, family in enumerate(groups_cal):
            if family in family_stats:
                stats = family_stats[family]
                y_cal_pred_destd[i] = y_cal_pred_std[i] * stats['std'] + stats['mean']
            else:
                y_cal_pred_destd[i] = y_cal_pred_std[i] * np.std(y_subtrain_cap) + np.mean(y_subtrain_cap)
        
        # Unclamp calibration predictions
        y_cal_pred_unclamp = np.maximum(y_cal_pred_destd, y_cal)
        
        # Calculate residuals on calibration set
        resid_cal = np.abs(y_cal - y_cal_pred_unclamp)
        ...
        # Predict on test set
        y_test_pred_std = rf.predict(X_test_processed)
        
        # De-standardize test predictions
        y_test_pred_destd = np.zeros_like(y_test_pred_std)
        for i, family in enumerate(groups_test):
            if family in family_stats:
                stats = family_stats[family]
                y_test_pred_destd[i] = y_test_pred_std[i] * stats['std'] + stats['mean']
            else:
                y_test_pred_destd[i] = y_test_pred_std[i] * np.std(y_subtrain_cap) + np.mean(y_subtrain_cap)
        
        # Unclamp test predictions
        y_test_pred_unclamp = np.maximum(y_test_pred_destd, y_test)
        
        # Convert to original scale
        y_test_pred_orig = np.expm1(y_test_pred_unclamp)
...
def calculate_stabilized_metrics(cv_results, all_predictions, all_true, y_original):
    """Calculate overall metrics and CQR-specific metrics"""
    print("\n=== CALCULATING STABILIZED METRICS ===")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='FP-DESIGN v2.2.2 Stabilized')
    parser.add_argument('--data', required=True, help='Path to balanced training data CSV')
    parser.add_argument('--out', required=True, help='Output directory')
    
    args = parser.parse_args()
    
    print("=== FP-DESIGN v2.2.2 STABILIZED (WINSORIZATION + FAMILY STANDARDIZATION) ===")
    ...
    # Train model with stabilization CV
    cv_results, all_predictions, all_true, all_families, all_folds = train_model_with_stabilization_cv(X, y_log, groups, sample_weights)
    
    # Calculate stabilized metrics
    metrics = calculate_stabilized_metrics(cv_results, all_predictions, all_true, y_original)
